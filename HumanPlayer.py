# CS76 Problem Assignment 2 10/1/2020
# Authors: Sudharsan Balasubramani & Alberto
# Collaborators: Discussed strategies with James Fleming, Mack Reiferson

import chess


# A class to track Human player moves for the Chess game.
class HumanPlayer():
    def __init__(self):
        print("Moves can be entered using four characters. For example, d2d4 moves the piece "
              "at d2 to d4.")
        pass

    # Generate list of legal moves. Then get move from stdin, determine if stdin is valid.
    def choose_move(self, board):
        moves = list(board.legal_moves)

        uci_move = None

        # While a move is not in moves, ask user to enter move, try it and if it isn't in moves, tell not legal.
        while uci_move not in moves:
            print("Please enter your move: ")
            human_move = input()

            try:
                uci_move = chess.Move.from_uci(human_move)
            except:
                # illegal move format
                uci_move = None

            if uci_move not in moves:
                print("  That is not a legal move!")

        print(uci_move in moves)

        return uci_move
